		       ...
		       const Char_t *trkFileName = "mfttracks.root")
{
  ...
  std::unique_ptr<TH2F> MFTTrackEtaPhi = std::make_unique<TH2F> ("MFT_EtaPhi", "Reconstructed Track", 100, -8, 8, 100, etaMin, etaMax);
  MFTTrackEtaPhi->GetXaxis()->SetTitle("Phi");
  MFTTrackEtaPhi->GetYaxis()->SetTitle("Eta");
...
//  o2::dataformats::MCTruthContainer<o2::MCCompLabel>* trkLabels = nullptr;
  ...
  for (Int_t event = 0; event < nEvents ; event++) {
	...
	nMFTDisksHasHits+= int(mcTrackHasHitsInMFTDisks[trkID][disk]);
	  ...
	  MFTTrackEtaPhi->Fill(thisTrack->GetPhi(), eta);
          if( (z > -5) & (z < 5) ) {
  ...
  MFTTracksp->Write();
  MFTTrackEta->Write();
  MFTTrackEtaPhi->SetOption("CONT4");
  MFTTrackEtaPhi->Write();
